...
{
    int cwnd = MAX_WINDOW_SIZE; // Initialize the sending window size
    ...
    while(FileNotEnd)
    {
        ...
        while(recvack<cwnd)
        {
            ...
            int timeo = -1;
            while ( (recvack < cwnd) && (timeo = readable_timeo(server_socket, 0, 500120) > 0) ) 
            {
                Packet ack;
...
int main(int argc, char *argv[])
{
    ...
    for(;;)
    {
        ...
        FILE *fp = NULL;
        if ( WaitShakeHands(file_name, &fp)>0 )
        {
            Packet req;
